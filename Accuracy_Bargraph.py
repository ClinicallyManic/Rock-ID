...
def Plot_AccurateVTotal():
    ...
    tp = np.diag(cm)
    print(np.sum(tp)/np.sum(test_totals))
    #Graphing
    fig, ax = plt.subplots()
    fig.set_size_inches(8,7)
    ax.barh(test_labels, (test_totals - tp)/test_totals, label='Total - TP', color='purple')
    ax.barh(test_labels, tp/test_totals, label='TP', left=(test_totals - tp)/test_totals, color='skyblue')
    ax.set_xlabel('%')
    ax.set_ylabel('Class')
